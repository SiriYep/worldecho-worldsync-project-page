...
- `action-coverage.css`: the original paper Figure 5c action-space coverage figure; interactive grouping awaits its source data.
...
The action-coverage section now shows the original paper Figure 5c, preserving its five action-query categories, PCA axes, and expert/off-expert 95% HDR contours. The previous model-mean charts were removed after the user clarified that the requested interaction concerns action-query coverage. **Group highlighting and the interactive action distribution are not complete.** The local PDF contains a flattened 141×104 point-cloud image, and no original point table or plotting script was found in the local paper history. Server inspection is pending because the known SSH endpoints timed out. See [action-coverage provenance and required source data](docs/action-coverage.md).

The fourth-part rollout gallery compares existing ground-truth videos with recorded rollouts from WorldSync and other models. It is not tied to 50 tasks, a particular training checkpoint, an AFE/state-control demonstration, or live inference. The slots remain marked pending until the actual matched recordings arrive. Minimum materials are case/scene labels, the GT video, and the corresponding videos with model names; see [rollout comparison materials](docs/rollout-comparisons.md). The separate GitHub/Hugging Face release retains its previously agreed 50-task-trained checkpoint and minimal single-task inference scope.
