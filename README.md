...
The fourth-part rollout gallery now contains six matched cases: recovered simulator GT, twelve selectable baseline configurations, and WorldSync (84 recordings in total). The supplied pack’s repeated-image GT files were removed; one cross-state replay case is omitted because its original GT video is unavailable. The remaining cases cover four query types and six subtypes, with training regimes, source frame rates and full sample IDs available. Playback follows normalized clip progress; LingBotVA is explicitly video-only. The gallery uses recorded outputs and is separate from live inference. See [media provenance and local preview instructions](docs/rollout-demo.md). The separate GitHub/Hugging Face release retains its previously agreed 50-task-trained checkpoint and minimal single-task inference scope.
