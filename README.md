...
The action-coverage section renders Figure 5c with five category controls and Action queries / Density coverage views. Selecting a category or a point highlights the complete category; selecting it again or All restores the full view. Both views preserve the source PCA coordinates and display bounds. The figure covers the single `adjust_bottle` task: 50 samples per category, the paper's deterministic 20-point display subset, and 95% HDR estimated from all samples. The off-expert outline is the union of four independently estimated family regions. The original paper image remains visible if the data cannot load. See [source, reproduction, and interaction details](docs/action-coverage.md).
