...
The results explorer reuses these seven rows as its only numerical source. Its scatter plot compares raw/gated trajectory error with visual pass; its distribution view summarizes the seven model averages. Both charts and their legend use transparent organization logos, with short labels distinguishing models that share a logo. Connecting lines preserve exact data positions when logos need space. Metric changes and model selection update the charts, with keyboard controls and reduced-motion support. These are **distributions across model means**, not per-task errors or a reconstruction of the paper’s action-space PCA. No synthetic samples or supplementary four-task results are mixed in. See [result provenance](docs/results-tables.md).
