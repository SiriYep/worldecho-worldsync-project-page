...
The fourth-part rollout gallery shows six selected tasks, with GT and WorldSync beside one of three comparison models: Cosmos-Predict2.5, CtrlWorld or DreamDojo. The three baselines use Expanded configurations; the selectors show task and model names only. Play/Pause, Restart and a common seek slider control the three videos. Each gate shows its recorded score and minimum requirement, with a binary arm-integrity exception and unscored GT reference. Beneath each model video, Pose DTW, position in cm and rotation in degrees compare the displayed AnyPos track with the same GT reference. These 24 demo-only comparisons use the main50 pose-DTW path without a gate penalty; lower baseline values remain visible. All 30 displayed recordings have calibrated AnyPos trajectory overlays, with an on/off control. A separate Real-world experiments section contains two cards labeled Video forthcoming.
