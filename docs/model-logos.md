...
Model names are read from explicit `data-lb-name` elements. Logo alternative text and affiliation names never enter the model IDs, rankings, or chart values. Scatter points keep their per-model colors because several models share the same developer.
