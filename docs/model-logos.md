...
LingBotVA uses the compact blue Ant Group mark at the user's request. Ant Group is the parent company of Robbyant, the model's developer; the hover title makes that relationship explicit. The original 80×80 PNG is the favicon loaded by the official Ant Group homepage, with a transparent outer background. It replaces the Robbyant wordmark for readability in the leaderboard. The model link still opens the official Robbyant project.
...
Model names are read from explicit `data-lb-name` elements. Logo alternative text and affiliation names never enter the model IDs, rankings, or chart values. The model-mean plots were removed after the user clarified that the requested interactive plot concerns action categories. Organization marks remain in the leaderboard; they are not the category markers in the paper's action-space plot.
