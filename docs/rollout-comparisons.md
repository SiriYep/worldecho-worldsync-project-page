...
The section now displays seven supplied cases with ground truth, twelve selectable baseline configurations, and WorldSync. The `#state-control` fragment remains for link compatibility. See [the current provenance, selection, playback and preview documentation](rollout-demo.md).
