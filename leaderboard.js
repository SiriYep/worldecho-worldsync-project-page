...
export function setupLeaderboard(root) {
  ...
  function update() {
    ...
    for (const { model, rank } of rankModels(models, metric)) {
      ...
      row.cells[{ gated: 3, raw: 4, visual: 5 }[metric]].classList.add("lb-primary");
      body.append(row);
    }
    ...
    root.querySelector("caption").textContent = `Published WorldEcho main comparison: ${models.length} models across 50 RoboTwin tasks, ranked by ${METRICS[metric]}.`;
    root.querySelector("[data-lb-status]").textContent = `${models.length} models · ranked by ${METRICS[metric]}`;
  }
