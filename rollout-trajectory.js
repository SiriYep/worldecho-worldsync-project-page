  ...
  function draw() {
    ...
    ["left", "right"].forEach((arm, armIndex) => {
      const trace = (through, alpha, lineWidth) => {
        ...
        ctx.strokeStyle = COLORS[armIndex]; ctx.globalAlpha = alpha;
        ctx.lineWidth = lineWidth; ctx.lineCap = "round"; ctx.lineJoin = "round"; ctx.stroke();
      };
      trace(data.frames.length - 1, 0.23, 1.8);
      trace(index, 0.95, 2.1);
      ...
      ctx.fillStyle = COLORS[armIndex]; ctx.fill(); ctx.strokeStyle = "#ffffff"; ctx.lineWidth = 1.1; ctx.stroke();
      ...
      if (Array.isArray(axes) && axes.length === 3) {
        axes.forEach((end, axis) => {
          ...
          ctx.strokeStyle = ["#ed4858", "#42bc71", "#448bed"][axis]; ctx.lineWidth = 1.3; ctx.stroke();
        });
      }
    });
    ctx.restore();
