...
export function setupActionCoverage(root, payload) {
  ...
  const status = host("data-ac-status", "ac-status");
  const source = host("data-ac-source-label", "ac-source-label");
  ...
  if (data.source.url) {
    ...
    link.setAttribute("href", data.source.url);
  ...
  if (hasDensity) {
    const views = host("data-ac-views", "ac-views");
    views.setAttribute("role", "group");
    views.setAttribute("aria-label", "Action coverage view");
    views.replaceChildren();
    for (const [id, name] of [["points", "Action queries"], ["density", "Density coverage"]]) {
      const button = doc.createElement("button");
      button.type = "button";
      button.className = "ac-view-button";
      button.textContent = name;
      button.dataset.acView = id;
  ...
  function renderSelection() {
    for (const group of data.groups) {
      ...
      cloud.setAttribute("visibility", view === "density" ? "hidden" : "visible");
      chart.append(cloud);
    }
    ...
    for (const path of regions.values()) {
      ...
      path.setAttribute("fill-opacity", view === "density" ? ".14" : ".045");
    }
    for (const [id, path] of familyRegions) {
      path.setAttribute("visibility", selected === id ? "visible" : "hidden");
      path.setAttribute("fill-opacity", view === "density" ? ".24" : ".06");
    }
    ...
    status.textContent = view === "density"
      ? group ? `${group.label}: 95% density region highlighted, estimated from all ${group.totalCount ?? group.points.length} samples.`
        : "Expert 95% density region and the union of the four off-expert regions, estimated from all source samples."
      : group ? `${group.label}: ${group.points.length} points highlighted${group.totalCount ? ` from ${group.totalCount} samples` : ""}. Other categories are dimmed.`
    ...
    chart.setAttribute("aria-label", `${data.source.label}. ${xLabel}; ${yLabel}. ${status.textContent}`);
  }
  ...
  for (const [id, button] of viewButtons) button.addEventListener("click", () => setView(id));
  renderSelection();
  root.hidden = false;
  return { select, getSelection: () => selected, setView, getView: () => view, data };
}
